...
Describe 'Remove-StaleFiles' {
	...
	BeforeEach {
		...
		$null = New-Item -Path $OldDir -ItemType Directory
	...
	Context 'File Detection' {
		...
		It 'Should handle empty directories' {
			...
			$null = New-Item -Path $EmptyOldDir -ItemType Directory
	...
	Context 'System Path Safety Checks' {
		BeforeAll {
			...
			$null = New-Item -Path $MockSystemPath -ItemType Directory -Force

			# Extract Test-SystemPath function for direct testing
			$FunctionContent = Get-Content ('{0}\..\functions\Remove-StaleFiles.ps1' -f $PSScriptRoot) -Raw
		...
		It 'Should prevent execution on system paths without AllowSystemPaths' {
			# Mock the function to always return true (simulate system path detection)
			InModuleScope PSF {
	...
	Context 'WhatIf Integration Tests' {
		...
		It 'Should work with WhatIf and AllowSystemPaths on safe test directory' {
			...
			$null = New-Item -Path $SafeTestDir -ItemType Directory -Force
